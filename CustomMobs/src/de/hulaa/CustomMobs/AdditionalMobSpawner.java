...
public class AdditionalMobSpawner {
	...
	public static Location getSpawnLocation(Player ChoosenPlayer) {
		...
		if (SpawnLoc.getBlockY() < world.getHighestBlockYAt(SpawnLoc)) {
			...
			double xAdjustment = (double) Utils.randInt(-75, 75);
			double zAdjustment = (double) Utils.randInt(-75, 75);
	...
	public static EntitySkeleton spawnSkeleton (net.minecraft.server.v1_9_R1.World nmsWorld,int spawnLevel) {
		...
		try {
			...
			Constructor mobConstructor = mobClass.getConstructor(net.minecraft.server.v1_9_R1.World.class);
