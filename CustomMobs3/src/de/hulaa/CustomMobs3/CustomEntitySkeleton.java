...
//	            		i=this.getExpValue(this.killer);
//	            		int numberOfOrbs = i;
//	            		Bukkit.getServer().broadcastMessage("[BETA] Skeleton xp distribution");
//	            		
//	            		World world = this.getWorld();
//	                    org.bukkit.World BukkitWorld = world.getWorld();
//	                    Location loc = new Location (BukkitWorld, this.locX, this.locY, this.locZ);
//	            		
//	            		for(; numberOfOrbs>=0; numberOfOrbs--){
//
//	            			Entity orb = new CustomEntityExperienceOrb(this.world);
//	 	                    AdditionalMobSpawner.spawnEntity(orb, loc );
//	 	                    
...
//
//	            this.die();
//	            Bukkit.getServer().broadcastMessage("[BETA] Skeleton die()");
//
////	            for (int i = 0; i < 20; ++i) {
////	                double d0 = this.random.nextGaussian() * 0.02D;
////	                double d1 = this.random.nextGaussian() * 0.02D;
////	                double d2 = this.random.nextGaussian() * 0.02D;
////	                this.world.addParticle(EnumParticle.FIREWORKS_SPARK, this.locX + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, this.locY + (double) (this.random.nextFloat() * this.length), this.locZ + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, d0, d1, d2);
////	            }
		...
		//EQUIPMENT PROCESS
		//if(MobLevel>0){
			ItemStack[] equipment = EquipmentGenerator.generateEquipment(11);
			for (int i = 0; i<5; i++){
		...
		//}else{
			//Bukkit.getServer().broadcastMessage("[BETA] Skeleton Level was not greater 0 :(");
...
//		super.setCustomName("StandardSkeleton <"+levelString+">");
		...
		protected void initAttributes() {
			...
			this.getAttributeInstance(GenericAttributes.FOLLOW_RANGE).setValue(15.0D);
